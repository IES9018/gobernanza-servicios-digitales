# Declaración de Responsabilidad — Servicio Digital IES 9-018

**Versión:** v0.9 — Junio 2026
**Documento relacionado:** `04_DECLARACION_RESPONSABILIDAD.md`

Yo, **{nombre del responsable}**, DNI **{número}**, responsable del proyecto
**{nombre del proyecto}**:

**Declaro** ser el único responsable del contenido, funcionamiento y datos
del servicio.

**Garantizo** que el servicio no vulnera derechos de autor, privacidad ni
normativa aplicable.

**Libero** al IES 9-018, sus autoridades, docentes y administradores
técnicos de toda responsabilidad derivada del servicio.

**Acepto** la suspensión inmediata del servicio ante incumplimientos.

---

| Firma del responsable | Firma institucional |
|-----------------------|---------------------|
| | |
| Nombre: | Nombre: |
| Fecha: | Fecha: |
